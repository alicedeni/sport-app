...
const ProfileData = ({
  ...
  editModeProfile,
  handleInputChange,
  handleCancelClickProfile,
  handleSaveClickProfile,
    ...
    <div className="profile-block-content-data">
      <div className="profile-block-content-data-title">
        ...
        {editModeProfile ? (
          <>
            <button
              onClick={handleEditClickProfile}
      ...
      <div className="profile-block-content-data-items">
        ...
        <div className="profile-block-content-data-item">
          ...
          <div
            className="profile-block-content-data-item-oval"
            style={{
              backgroundColor: bmi ? getColorCode(bmi) : '#B0B0B0',
              borderColor: bmi ? getColorCodeText(bmi) : '#B0B0B0',
            }}
          >
            {bmi !== null && bmi != 0 ? (
              <p
                className="profile-block-content-data-item-oval-text"
                style={{ color: getColorCodeText(bmi) }}
              >
                {bmi}
              </p>
            ) : (
              <p className="profile-block-content-data-item-oval-text" style={{ color: '#B0B0B0' }}>
                -
              </p>
            )}
          </div>
      ...
      {editModeProfile && (
        <div className="profile-block-content-data-btn">
          <button
            onClick={handleCancelClickProfile}
            className="profile-block-content-data-btn-cancel"
          >
            Отменить
          </button>
          <button onClick={handleSaveClickProfile} className="profile-block-content-data-btn-save">
            Сохранить
          </button>
        </div>
      )}
