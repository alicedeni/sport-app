...
const TeamModal = ({ team, onClose, participants, teamColor }) => {
  ...
  const getTeamMembersWithRankings = () => {
    const teamMembers = participants.filter((participant) => participant.team === team.name)

    const sortedTeamMembers = teamMembers.sort((a, b) => b.progress - a.progress)

    const sortedAllParticipants = participants.sort((a, b) => b.progress - a.progress)

    return sortedTeamMembers.map((member, teamIndex) => {
      const globalIndex = sortedAllParticipants.findIndex((p) => p.id === member.id)
      return {
        ...member,
  ...
  const teamMembersWithRankings = getTeamMembersWithRankings()
      ...
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-content__description">
          ...
          <div className="team-members-list">
            {teamMembersWithRankings.map((member) => (
              <div key={member.id} className="team-member-item">
                <div className="team-member-info">
                  <div className="team-member-name">
                    {member.lastName} {member.firstName}
                  </div>
                  <div className="team-member-progress flex-center-vertical">
                    <div className="team-member-fire-count">{member.progress}</div>
                    <svg
                      width="16"
                      height="16"
                      viewBox="0 0 32 32"
                ...
                <div className="team-member-rankings">
                  <div className="team-member-ranking">
                    <span className="ranking-label">Команда:</span>
                    <span className="ranking-position">#{member.teamPosition}</span>
                  </div>
                  <div className="team-member-ranking">
                    <span className="ranking-label">Общий:</span>
                    <span className="ranking-position">#{member.globalPosition}</span>
                  </div>
                </div>
              </div>
            ))}
